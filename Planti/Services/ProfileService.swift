...
class ProfileService {
    ...
    func completeMissionTarget(key: String,  completion: @escaping (Result<MyMissionTargetCompleteStatus,Error>)->Void,
                               urlResponse: @escaping (HTTPURLResponse) -> Void){
        
        let endpoint = Endpoint.profileMeMissionsTargetComplete(key: key)
        ...
        session.dataTask(with: request){ (data, response, error) in
            ...
            do {
                ...
                let missionComplete = try JSONDecoder().decode(MyMissionTargetCompleteStatus.self, from: data)
                DispatchQueue.main.async {
                    completion(.success(missionComplete))
                }
